...
void addPatient(struct Patient patient[], int max) 
{
    ...
    if (max > StructLength) 
    {
        patient[StructLength].patientNumber = nextPatientNumber(patient, max);
        inputPatient(patient + StructLength);
...
void inputPatient(struct Patient* patient) 
{
    ...
    for (i = 0; i < strlen(PatientName) - 1; i++) 
    {
        patient->name[i] = PatientName[i];
    }
...
void inputPhoneData(struct Phone* phone) 
{
    ...
    case 1:
        ...
        {
            phone->number[i] = UserPhoneNumber[i];
        }
